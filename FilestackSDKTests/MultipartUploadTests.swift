...
class MultipartUploadTests: XCTestCase {
  ...
  func testCancellingResumableMultiPartUpload() {
    ...
    let expectation = self.expectation(description: "request should succeed")
    let progressExpectation = self.expectation(description: "progress should finish")
    
    var error: Error!
    
    let progressHandler: ((Progress) -> Void) = { progress in
      if progress.completedUnitCount == MultipartUploadTests.sampleFileSize {
        progressExpectation.fulfill()
      }
    }
    
    let multipartUpload = client.multiPartUpload(from: sampleFileUrl, uploadProgress: progressHandler) { (resp) in
      error = resp.error
      expectation.fulfill()
    }
